...
export const { template, render } = make([
  ...
  (text, token = Object.freeze({ literal: text })) =>
    (scope, acc) =>
      acc.push(token),
  // A tagged interpolation spreads what the compile's `tag` returned, with
  // the value written last so the token's own key cannot be taken over. The
  // branch is taken once, at compile time.
  (expr, tagged) =>
    tagged
...
export const text = (tokens) =>
  // Widened because each token carries one key or the other, which the public
  // union deliberately does not model.
