...
export const relocate = (diag, { prefix = "", offset = 0 } = {}) => {
  ...
  let d = /** @type {any} */ (diag),
    copy;
  if (isXprsnDiagnostic(d)) copy = relocateXprsn(d, { prefix, offset });
  else {
    const props = DESCS(d);
    ...
    props.start && ((props.start.value += offset), (props.end.value += offset));
    DEFINE(copy, props);
  }
  // One registration for both halves: the copy joins the store under the
  // original's origin either way.
 ...
 * branch and then has to see the closer itself; anything but the closer here is
 * an unexpected tag either way.
 *
 * @param {string} close
 * @param {Node<any>[]} [nodes]
 * @returns {Node<any>[]}
...
let elseTail = (close, nodes) =>
  last[1] === "#else"
    ? // oxlint-disable-next-line no-unused-expressions
      ((nodes = parse([close])), last[1] === close || unexpected(last), nodes)
    : last[1] === close
      ? []
      : unexpected(last);
 ...
 */
let branch =
  (cond, then = parse(["#elif", "#else", "/if"]), els = elseChain(last[1])) =>
  (scope, acc) =>
    run(cond(scope) ? then : els, scope, acc);

/**
 * The pairs one `#each` walks. The caller has already asked whether the
 * collection is an array — that answer decides the key shape too, so it is
 * passed in rather than asked twice.
 *
 * @param {any} listValue
 * @param {boolean} arr
 * @returns {any[]}
...
let eachPairs = (listValue, arr) =>
  arr
    ...
    : listValue && typeof listValue === "object"
      ? Object.keys(listValue).map((k) => [listValue[k], k])
      : [];
 ...
 */
export const litNode = (text) => (scope, acc) => (acc.text += text);
