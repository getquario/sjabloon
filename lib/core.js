...
let lexStep = (a = source.indexOf("{{", i)) => {
  if (a >= 0) {
    ...
    // A raw opener with no `}}}` left in the source re-reads as a normal tag,
    // and the latch stays off so the rest of the scan skips the triple probe.
  ...
  // final text token.
  if (a < 0 || lxB < 0) return (tokens.push([0, source.slice(i)]), 0);
