 ...
 * @returns {never}
 */
const fault = (msg, code, t, start = t?.[2] ?? source.length, end = t?.[3] ?? source.length) => {
 ...
 * @param {any} acc
 */
let run = (nodes, scope, acc) => { for (const n of nodes) n(scope, acc); };

// A leaf interpolation node: compile the expression, then let the profile turn
// the evaluated value into output. `build` is VAL for `{{ }}`, RAW for `{{{ }}}`.
/**
 * @param {Tok} token
 * @param {any} build
 * @returns {Node<any>}
 */
let interp = (token, build) => build(compileExpr(token[1], token[4], snap()));
...
let compileExpr = (expr, start, context) => {
	...
	for (const n of e.names) bound.includes(n) || names.add(n); // oxlint-disable-line no-unused-expressions
	...
	for (let t; (t = tokens[i++]); ) {
		...
		if (!t[0]) {
			...
			nodes.push((scope, acc) => {
				const listValue = list(scope), arr = Array.isArray(listValue);
				const pairs = arr ? listValue.slice() : listValue && typeof listValue === 'object' ? Object.keys(listValue).map(k => [listValue[k], k]) : [];
				...
				pairs.forEach((x, j) => {
					...
					child['@'] = item;
					child.loop = { index: j + 1, index0: j, first: !j, last: j === pairs.length - 1, length: pairs.length };
					run(body, child, acc);
				});
			});
...
export let make = ([lit, val, raw, seed, take]) => {
	...
	function template(str, funcs) {
		...
		const f = (/** @type {any} */ values, /** @type {any} */ anchors) => {
			...
			if (!anchors) r['@'] = values;
			else if ('item' in anchors) r['@'] = anchors.item;
