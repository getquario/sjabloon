...
2. **Escaping lives in `lib/html.js` and nowhere else.** That edition must HTML-escape `{{ }}` (`& < > " '`) with raw output only via explicit `{{{ }}}` — never flip that default *there*. The other two editions are output-neutral by design and escape nothing; do not "helpfully" add escaping to them, and do not let `esc` drift back into `core.js`.
...
- **Don't try to generate the `.d.ts`.** Three independent routes are closed, so this keeps getting reattempted and keeps failing:
  1. `isolatedDeclarations` rejects `export const { template, render } = make(...)` with TS9019 — and that destructured export *is* the `make(profile)` architecture.
  2. Plain declaration emit resolves type imports against the *runtime* graph, so a JSDoc-typedef-only module exports nothing. It does not fail loudly: it emits `undefined` in their place (`SjabloonRenderer<Token[]>` becomes `undefined<undefined[]>`) and exits 0.
