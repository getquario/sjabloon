...
- `npm test` — Node's built-in test runner under `--disallow-code-generation-from-strings` (strict-CSP simulation), then `npm run test:types`, a plain `tsc` that type-checks `src/` **and** `test/types.check.ts` in one pass. No build needed: `types.check.ts` imports `../src/*.js` directly, so the whole suite stays buildless. Keep this on Node: Bun accepts that V8 flag but does not enforce it.
- `npm run build` — tsdown (rolldown + oxc), configured in `tsdown.config.js` → `dist/` (ESM only, ES2024), three entries plus one shared `dist/core.js` chunk. Type generation is off (`dts: false`); the hand-written `src/*.d.ts` are carried over by tsdown's `copy`. `xprsn` stays external (a runtime dependency, not bundled). `hash: false` and `copy` are the only non-default knobs, so the chunk name stays `core.js` and the size budgets and browser harness can name it.
...
- `dist/` is gitignored build output and the **only** thing published — `files` is just `["dist"]`. `src/` does not ship: the exports map seals the package, so a shipped `src/` would be 20 kB nobody can import.
- The `.d.ts` are **hand-written and live in `src/`**, beside the code they describe, and tsdown's `copy` carries them into `dist/`. Shared types live once in `src/types.d.ts`; `index`/`text`/`html` re-export it with `export * from './types.js'` and declare only their own `template`/`render` return types. Hang prose on the shared types, not on each `template`, or it multiplies by three. `test/types.check.ts` covers all three entries by importing `../src/*.js` directly and pins the return types with `@ts-expect-error`. It deliberately does **not** self-reference the package, which keeps the suite buildless — the cost is that nothing checks the `exports` map's `types` conditions or that `copy` actually landed the `.d.ts` in `dist/`.
- **`checkJs` over `src/` is what keeps those declarations honest**, and it replaced a test. `tsconfig.json` type-checks `src/**/*.js` with `noImplicitAny` off — that one setting is the difference between 15 real errors and 119 mostly-noise ones. `fault()` takes `SjabloonErrorCode`, so a code the parser throws but `types.d.ts` omits fails to compile. Verify with: swap a code in `src/core.js` for a made-up one and confirm `npm run test:types` fails.
- **Don't try to generate the `.d.ts` with tsdown.** Both routes are closed. `isolatedDeclarations` rejects `export const { template, render } = make(...)` with TS9019, and that destructured export *is* the `make(profile)` architecture. The TypeScript route resolves type imports against the *runtime* graph, so a JSDoc-typedef-only module exports nothing — and it does not fail, it emits `undefined` in their place (`SjabloonRenderer<Token[]>` becomes `undefined<undefined[]>`) and exits 0. Neither is a TypeScript 7 problem: TS 5.9 behaves identically.
- Tests that scan sources use `globSync('src/**/*.js')`, never a hardcoded filename — a scan pinned to one path keeps passing after a module moves, covering nothing. The call site already fails loudly on an empty match, so the glob needs no extra guard.
