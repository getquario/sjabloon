...
- `renderer.slots` is one entry per interpolation a registry function answers, in source order; a block's own expression renders no token and holds none. Each evaluates that expression alone, so an embedder can settle an async call before rendering and hand the answer back as `scoped(values, supply)`. `supplied` is module-level render state written by every `scoped` call, so a stale one is never read; `slot in supplied` is the test, which keeps a supplied `undefined` a value rather than a hole.
...
- Diagnostics are minted, authenticated and relocated through [waarmerk](https://github.com/getquario/waarmerk), which xprsn, treffer and padvinder share. `store()` at module load annotated `Store<SjabloonErrorCode>`, `mint` to throw, `adopt` where a diagnostic joins this store as well as xprsn's, `relocate` re-exported with this module's store applied. Do not hand-roll a second copy of that machinery here, and do not drop the store's type argument: `store()` defaults `Code` to `string`, which compiles and checks nothing.
- `attach` is gone; `adopt` is waarmerk's version of it and takes the `blocks` context as a field. `fault` mints in one call, `blocks` included — the fields are defined by descriptor, so the frozen context stays frozen and non-writable through any number of relocations.
- `relocate` keeps its two halves. An xprsn diagnostic is copied by xprsn and then adopted here, so it stays authentic to both packages; anything this module minted goes straight through waarmerk's relocation. It also takes `span` as well as `offset`, for an embedder whose text reached the template through a decode.
...
- **Keep the one fact a reader needs at that line.** An invariant the code cannot state ("the timeout stays below the poll interval; the host kills longer waits") or a sync obligation with another file ("mirror the list in `lib/html.js`"). A comment that only restates a decision the code already reflects is deleted, even one that points at a doc.
