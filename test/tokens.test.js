...
test('literals and values interleave in render order', () => {
	...
	assert.deepStrictEqual(template('{{ amount }}')({ amount: 1000 }), [val(1000)], 'a bare interpolation is one value token');
	assert.deepStrictEqual(template('Total: {{ amount }}')({ amount: 1000 }), [lit('Total: '), val(1000)], 'distinguishable from this');
	assert.deepStrictEqual(template('{{ a }}{{ b }}')({ a: 1, b: 2 }), [val(1), val(2)], 'no empty literal between adjacent tags');
	assert.deepStrictEqual(template('  {{- a }}')({ a: 1 }), [val(1)], 'nor for a text run trimmed to nothing');
});
...
test('values are captured pre-stringify', () => {
	...
	assert.strictEqual(tokens[1].value, null, 'nullish is captured as-is, not the empty string it renders as');
...
test('blocks: per iteration, taken branches only, never block expressions', () => {
	...
	assert.deepStrictEqual(
		template('{{#each items as it}}{{ it }}{{#else}}{{ fallback }}{{/each}}')({ items: [], fallback: 'none' }),
		[val('none')],
...
test('every render owns its stream, including under re-entrancy', () => {
	...
	const seen = [];
	const funcs = {
		nested: v => { const t = inner({ x: v * 10 }); seen.push(t); return text(t); },
		throwing: () => { try { inner(); } catch { /* ignore */ } return 'ok'; },
	};
	assert.deepStrictEqual(
...
test('literal tokens are frozen and shared across iterations', () => {
	...
	assert.strictEqual(tokens[0], tokens[2], 'one hoisted object per text node, not one per iteration');
...
test('stringification is deferred, so text() fails where the stream did not', () => {
	...
	assert.throws(() => plain('{{ x }}')({ x: hostile }), TypeError, 'the string edition fails at render');
});
