...
test('if, elif, else', () => {
	...
	assert.strictEqual(render('{{#if a}}{{#if b}}both{{/if}}{{#else}}not a{{/if}}', { a: true, b: true }), 'both', 'nested');
	...
	assert.strictEqual(render('{{#if a}}{{#if b}}1{{#elif c}}2{{/if}}{{#elif d}}3{{/if}}', { a: true, c: true }), '2', 'nested chains');
});

test('each over arrays and objects', () => {
	assert.strictEqual(render('{{#each items as it}}[{{ it }}]{{/each}}', { items: [1, 2] }), '[1][2]');
	...
	assert.strictEqual(render('{{#each prices as p, sku}}{{ sku }}={{ p }};{{/each}}', { prices: { a1: 4, b2: 9 } }), 'a1=4;b2=9;');
...
test('each scopes and shadowing', () => {
	assert.strictEqual(render('{{#each items as it}}{{ prefix }}{{ it }} {{/each}}', { items: [1, 2], prefix: '#' }), '#1 #2 ');
	assert.strictEqual(
		render('{{#each rows as row}}{{#each row as cell}}{{ cell }},{{/each}};{{/each}}', { rows: [[1, 2], [3]] }),
		'1,2,;3,;',
		'nested each shadows correctly'
	);
	assert.strictEqual(render('{{#each xs as v}}{{ v }}{{/each}}{{ v }}', { xs: [1], v: 'outer' }), '1outer', 'parent value intact');
});

test('loop metadata', () => {
	assert.strictEqual(render('{{#each xs as x}}{{ x }}{{#if not loop.last}}, {{/if}}{{/each}}', { xs: ['a', 'b', 'c'] }), 'a, b, c');
	assert.strictEqual(render('{{#each xs as x}}{{ loop.index }}/{{ loop.length }}:{{ x }} {{/each}}', { xs: ['a', 'b'] }), '1/2:a 2/2:b ');
	assert.strictEqual(render('{{#each xs as x}}{{#if loop.first}}[{{/if}}{{ x }}{{#if loop.last}}]{{/if}}{{/each}}', { xs: [1, 2, 3] }), '[123]');
	assert.strictEqual(render('{{#each o as v}}{{ loop.index0 }}={{ v }};{{/each}}', { o: { a: 10, b: 20 } }), '0=10;1=20;');
	assert.strictEqual(
...
test('$ and @ scope anchors', () => {
	...
	assert.strictEqual(render('{{ $.title }}/{{ @.title }}', { title: 'T' }), 'T/T', 'both point at values at the root');
...
test('{ root, item } overrides the anchors without mutating anything', () => {
	assert.strictEqual(template('{{ $.a }}/{{ @.a }}')({}, { root: { a: 'R' }, item: { a: 'I' } }), 'R/I', 'distinct objects');
	assert.strictEqual(template('{{ $.y }}')({}, { root: { y: 'Z' } }), 'Z');
	assert.throws(() => template('{{ @.x }}')({}, { root: {} }), TypeError, 'omitting item leaves @ unbound');
	assert.strictEqual(
	...
	const bare = {}, root = { title: 'T' }, item = { n: 1 };
	template('{{ $.title }}/{{ @.n }}')(bare, { root, item });
	assert.deepStrictEqual([root, item, bare], [{ title: 'T' }, { n: 1 }, {}], 'an override mutates nothing either');
});
...
test('whitespace trimming and comments', () => {
	...
	assert.strictEqual(render('{{ -n }}', { n: 5 }), '-5', 'a dash away from the brace stays a unary minus');
...
test('compile once, render many, with a function registry', () => {
	...
	assert.strictEqual(render('{{ fmt(price) }}', { price: 4.5 }, { fmt: n => '€' + n.toFixed(2) }), '€4.50');
...
test('names', () => {
	...
	assert.deepStrictEqual(template('{{ x }}{{#each xs as x}}{{ x }}{{/each}}{{ y }}').names, ['x', 'xs', 'y'], 'free use outside still counts');
	assert.deepStrictEqual(
	...
	assert.deepStrictEqual(template('{{ $.a }}{{#each xs as x}}{{ @.b }}{{/each}}').names, ['xs'], 'anchors are excluded');
	assert.deepStrictEqual(template('{{#each xs as x}}{{ loop.index }}{{/each}}').names, ['xs'], 'loop is engine-bound inside a loop');
	assert.deepStrictEqual(template('{{ loop }}').names, ['loop'], 'and an ordinary name outside one');
