...
test('escaping is html only', () => {
	...
	assert.strictEqual(html.render('{{ x }}', { x: '<script>alert(1)</script>' }), '&lt;script&gt;alert(1)&lt;/script&gt;');
	assert.strictEqual(plain.render('{{ x }}', evil), '<b>&"\'</b>', 'the text edition is output-neutral');
	assert.strictEqual(root.text(root.render('{{ x }}', evil)), '<b>&"\'</b>', 'and so is the token edition');
});

test('{{{ }}} is raw in html and a located error everywhere else', () => {
	assert.strictEqual(html.render('{{{ x }}}', { x: '<b>bold</b>' }), '<b>bold</b>');
	assert.strictEqual(html.render('[{{{ missing }}}]', {}), '[]', 'nullish is empty through the raw form too');
	assert.strictEqual(html.render('a {{{- html -}}} b', { html: '<i>' }), 'a<i>b', 'raw tags trim too');
...
test('text(tokens) equals the plain-string render', () => {
	...
	const VALUES = [
		{},
		{ x: 'v', n: 1000, d: null, o: { a: 1 }, ok: true, alt: true, a: 'A', b: 'B', c: 'C', xs: [1, 2], t: 'T', fallback: 'F', rows: [{ n: 1, cs: ['p'] }] },
	...
	for (const src of CASES) {
		for (const values of VALUES) {
			assert.strictEqual(root.text(root.template(src, funcs)(values)), plain.template(src, funcs)(values), src);
		}
	}
});
