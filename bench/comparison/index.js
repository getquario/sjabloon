...
const templates = {
  sjabloon: {
    ...
    escaped:
      "<ul>{{#each list as item}}<li>User: {{ item.user }} / Web Site: {{ item.site }}</li>{{/each}}</ul>",
  },
  tempura: {
    raw: "<ul>{{#expect list}}{{#each list as item}}<li>User: {{{ item.user }}} / Web Site: {{{ item.site }}}</li>{{/each}}</ul>",
    escaped:
      "<ul>{{#expect list}}{{#each list as item}}<li>User: {{ item.user }} / Web Site: {{ item.site }}</li>{{/each}}</ul>",
  },
  handlebars: {
...
function expected(list, mode) {
  ...
  return (
    "<ul>" +
    list
      .map((item) => `<li>User: ${value(item.user)} / Web Site: ${value(item.site)}</li>`)
      .join("") +
    "</ul>"
  );
}
