...
export function renderSafe(src, values, funcs) {
  ...
  if ((tokenErr === undefined) !== (strErr === undefined))
    throw new Error("text(tokens) and the string render disagree on failure");
  if (tokenErr) return;
  ...
  for (const t of tokens) {
    ...
    if (!lit) continue;
    if (typeof t.literal !== "string" || !t.literal)
      throw new Error("empty or non-string literal token");
