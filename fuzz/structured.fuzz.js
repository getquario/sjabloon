...
function buildExpr(data, depth) {
  ...
  if (kind === 3)
    return `(${buildExpr(data, depth - 1)} ? ${buildExpr(data, depth - 1)} : ${buildExpr(data, depth - 1)})`;
  ...
  if (kind === 6) {
    const idx =
      data.consumeIntegralInRange(0, 4) === 0
...
function buildTemplate(data, depth, raw) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (kind === 0) out += `{{ ${buildExpr(data, depth - 1)} }}`;
      ...
      if (data.consumeBoolean())
        out += `{{#elif ${buildExpr(data, depth - 1)}}}${buildTemplate(data, depth - 1, raw)}`;
...
function buildValues(data, names) {
  ...
  for (const n of names) {
    ...
    if (pick === 0) vals[n] = null;
      ...
      vals[n] = Array.from({ length: data.consumeIntegralInRange(1, 3) }, () => ({
        price: data.consumeIntegralInRange(0, 9),
        qty: data.consumeIntegralInRange(1, 4),
      }));
  }
  return vals;
}
...
function rawlessPass(entry, src, values) {
  let render;
  try {
    render = entry.template(src, FUNCS);
  } catch (e) {
    ...
    else if (Object.hasOwn(e, "blocks"))
      throw new Error("unauthenticated error gained template context");
    return;
  }
  if (values === undefined) return;
  let out;
  try {
    out = entry.out(render(values));
  } catch (e) {
  ...
  if (ok) {
    ...
    try {
      ...
      assert.strictEqual(
        isDiagnostic(again),
        authenticated,
        "render diagnostic provenance changed",
      );
